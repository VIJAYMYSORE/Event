...
class Jsv4 {
	...
	private function checkTypes() {
		if (isset($this->schema->type)) {
			...
			$this->fail(JSV4_INVALID_TYPE, "", "/type", "Invalid type: $type for $d");
		}
	}
			...
			foreach ($this->schema->enum as $option) {
				if (self::recursiveEqual($this->data, $option)) {
					return;
                        ...
                        $d = json_encode($this->data);
                        $enum = json_encode($this->schema->enum);
			$this->fail(JSV4_ENUM_MISMATCH, "", "/enum", "Value $d is not in the list of enum options: $enum");
		}
	}
